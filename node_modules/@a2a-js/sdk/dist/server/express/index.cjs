...
async function* delegateAsyncIterator(it) {
  let nextThrew = false;
  try {
    while (true) {
      let result;
...
// src/server/transports/list_tasks_serializer.ts
function serializeListTasksResponse(response, options) {
  const serialized = ListTasksResponse.toJSON(response);
  const { tasks, nextPageToken, pageSize, totalSize, ...rest } = serialized;
  const serializedTasks = tasks ?? [];
  return {
    tasks: options?.includeArtifacts ? serializedTasks.map(withMaterializedArtifacts) : serializedTasks,
...
function withMaterializedArtifacts(task) {
  if (typeof task !== "object" || task === null) {
    return task;
  }
  const serializedTask = task;
  if (serializedTask.artifacts !== void 0) {
...
// src/compat/v0_3/translate/_validate.ts
...
function toCompatAgentCard(core, options) {
  ...
  const securitySchemes = core.securitySchemes && Object.keys(core.securitySchemes).length > 0 ? Object.fromEntries(
...
var V1_METHODS_WITHOUT_LEGACY_EQUIVALENT = /* @__PURE__ */ new Set(["ListTasks"]);
...
function isLegacyJsonRpcMethod(method) {
  return typeof method === "string" && method in LEGACY_JSONRPC_TO_V1;
}
function isV1JsonRpcMethod(method) {
...
function isLegacyRequest(body) {
  ...
  return isLegacyJsonRpcMethod(body.method);
}
function shouldUseLegacyFallback(body) {
  if (typeof body !== "object" || body === null) return false;
  return !isV1JsonRpcMethod(body.method);
}
function jsonRpcHandler(options) {
  const jsonRpcTransportHandler = new JsonRpcTransportHandler(options.requestHandler);
  const legacyJsonRpcTransportHandler = options.legacyCompat?.enabled ? new LegacyJsonRpcTransportHandler(options.requestHandler) : void 0;
  const router = import_express.default.Router();
  router.use(contentTypeGuard);
  router.use(import_express.default.json(), jsonErrorHandler);
  router.post("/", async (req, res) => {
    const useLegacy = legacyJsonRpcTransportHandler !== void 0 && (isLegacyRequest(req.body) || shouldUseLegacyFallback(req.body));
    const mapToError = useLegacy ? LegacyJsonRpcTransportHandler.mapToLegacyJSONRPCError : JsonRpcTransportHandler.mapToJSONRPCError;
    try {
      const user = await options.userBuilder(req);
      const requestedVersion = req.header(A2A_VERSION_HEADER) || void 0;
      const requestedExtensionsHeader = useLegacy ? req.header(LEGACY_HTTP_EXTENSION_HEADER) ?? req.header(HTTP_EXTENSION_HEADER) : req.header(HTTP_EXTENSION_HEADER);
      ...
      if (typeof rpcResponseOrStream?.[Symbol.asyncIterator] === "function") {
        ...
        const alwaysStreamSse = useLegacy && req.body?.method === LEGACY_METHOD_TASKS_RESUBSCRIBE;
        if (alwaysStreamSse) {
...
var contentTypeGuard = (req, res, next) => {
...
function computeETag(json) {
  const hash = import_crypto.default.createHash("sha256").update(json).digest("hex").slice(0, 16);
  return `W/"${hash}"`;
}
function legacyAgentCardRouter(options) {
    ...
    try {
      ...
      try {
        ...
        if (error instanceof VersionNotSupportedError) {
          ...
          res.status(HTTP_STATUS.BAD_REQUEST).setHeader("Content-Type", LEGACY_JSON_CONTENT_TYPE).json(toLegacyHTTPError(error));
          return;
        }
        throw error;
      }
      ...
      res.append("Vary", A2A_VERSION_HEADER);
      res.status(500).setHeader("Content-Type", LEGACY_JSON_CONTENT_TYPE).json({ error: "Failed to retrieve agent card" });
    }
  });
  return router;
}
...
// src/compat/v0_3/types/pb/a2a.ts
...
var Task3 = {
  fromJSON(object) {
    return {
      ...
      status: isSet2(object.status) ? TaskStatus2.fromJSON(object.status) : void 0,
...
var TaskStatus2 = {
  fromJSON(object) {
    return {
      ...
      update: isSet2(object.message) ? Message2.fromJSON(object.message) : isSet2(object.update) ? Message2.fromJSON(object.update) : void 0,
      timestamp: isSet2(object.timestamp) ? globalThis.String(object.timestamp) : void 0
    };
  },
  toJSON(message) {
    const obj = {};
...
var FilePart = {
  fromJSON(object) {
    return {
      file: isSet2(object.fileWithUri) ? { $case: "fileWithUri", value: globalThis.String(object.fileWithUri) } : isSet2(object.file_with_uri) ? { $case: "fileWithUri", value: globalThis.String(object.file_with_uri) } : isSet2(object.fileWithBytes) ? { $case: "fileWithBytes", value: Buffer.from(bytesFromBase642(object.fileWithBytes)) } : isSet2(object.file_with_bytes) ? { $case: "fileWithBytes", value: Buffer.from(bytesFromBase642(object.file_with_bytes)) } : void 0,
...
var TaskArtifactUpdateEvent2 = {
  fromJSON(object) {
    return {
      taskId: isSet2(object.taskId) ? globalThis.String(object.taskId) : isSet2(object.task_id) ? globalThis.String(object.task_id) : "",
      contextId: isSet2(object.contextId) ? globalThis.String(object.contextId) : isSet2(object.context_id) ? globalThis.String(object.context_id) : "",
      artifact: isSet2(object.artifact) ? Artifact2.fromJSON(object.artifact) : void 0,
...
var PushNotificationConfig = {
  fromJSON(object) {
    return {
      ...
      authentication: isSet2(object.authentication) ? AuthenticationInfo2.fromJSON(object.authentication) : void 0
    };
  },
  toJSON(message) {
    const obj = {};
...
var AgentCard2 = {
  fromJSON(object) {
    return {
      ...
      provider: isSet2(object.provider) ? AgentProvider2.fromJSON(object.provider) : void 0,
      ...
      capabilities: isSet2(object.capabilities) ? AgentCapabilities2.fromJSON(object.capabilities) : void 0,
...
var SecurityScheme2 = {
  fromJSON(object) {
    return {
      ...
      } : isSet2(object.mtlsSecurityScheme) ? { $case: "mtlsSecurityScheme", value: MutualTlsSecurityScheme2.fromJSON(object.mtlsSecurityScheme) } : isSet2(object.mtls_security_scheme) ? { $case: "mtlsSecurityScheme", value: MutualTlsSecurityScheme2.fromJSON(object.mtls_security_scheme) } : void 0
    };
  },
  toJSON(message) {
    const obj = {};
...
var OAuth2SecurityScheme2 = {
  fromJSON(object) {
    return {
      description: isSet2(object.description) ? globalThis.String(object.description) : "",
      flows: isSet2(object.flows) ? OAuthFlows2.fromJSON(object.flows) : void 0,
...
var SendMessageRequest2 = {
  fromJSON(object) {
    return {
      request: isSet2(object.message) ? Message2.fromJSON(object.message) : isSet2(object.request) ? Message2.fromJSON(object.request) : void 0,
      configuration: isSet2(object.configuration) ? SendMessageConfiguration2.fromJSON(object.configuration) : void 0,
      metadata: isObject2(object.metadata) ? object.metadata : void 0
    };
  },
  toJSON(message) {
...
var SendMessageResponse2 = {
  fromJSON(object) {
    return {
      payload: isSet2(object.task) ? { $case: "task", value: Task3.fromJSON(object.task) } : isSet2(object.message) ? { $case: "msg", value: Message2.fromJSON(object.message) } : isSet2(object.msg) ? { $case: "msg", value: Message2.fromJSON(object.msg) } : void 0
    };
  },
  toJSON(message) {
    const obj = {};
    if (message.payload?.$case === "task") {
      obj.task = Task3.toJSON(message.payload.value);
...
// src/compat/v0_3/types/converters/id_decoding.ts
...
// src/compat/v0_3/server/express/rest_handler.ts
var legacyRestErrorHandler = (err, _req, res, next) => {
  if (err instanceof SyntaxError && "body" in err) {
    return res.status(HTTP_STATUS.BAD_REQUEST).json(toLegacyHTTPError(A2AError2.parseError("Invalid JSON payload.")));
  }
  next(err);
};
function legacyRestRouter(options) {
  ...
  router.use(
    ...
    import_express3.default.json({ type: LEGACY_JSON_CONTENT_TYPE, strict: false }),
  ...
  const buildContext = async (req) => {
    ...
    const context = ctxBuilder({
      ...
      headers: req.headers,
      requestedVersion,
      tenant: req.params.tenant || void 0
    });
  ...
  const encodeSendMessageResult = (result) => {
    const protoResult = ToProto.messageSendResult(result);
    if (!protoResult) {
      throw A2AError2.internalError("sendMessage produced no result.");
    }
    return SendMessageResponse2.toJSON(protoResult);
  };
  const encodeTask = (task) => {
    return Task3.toJSON(ToProto.task(task));
  };
  const encodeTaskPushNotificationConfig = (cfg) => {
    return TaskPushNotificationConfig2.toJSON(ToProto.taskPushNotificationConfig(cfg));
  };
  const encodeAgentCard = (card) => {
    return AgentCard2.toJSON(ToProto.agentCard(card));
  };
  const encodeListTaskPushNotificationConfigs = (configs) => {
    return ListTaskPushNotificationConfigResponse.toJSON({
      configs: configs.map((c) => ToProto.taskPushNotificationConfig(c)),
      nextPageToken: ""
    });
  };
  const decodeSendMessageRequest = (rawBody) => {
    const proto = SendMessageRequest2.fromJSON(rawBody ?? {});
    return FromProto.messageSendParams(proto);
  };
  const decodeTaskPushNotificationConfig = (rawBody) => {
    const proto = TaskPushNotificationConfig2.fromJSON(rawBody ?? {});
    return FromProto.taskPushNotificationConfig(proto);
  };
  const encodeStreamEvent = (event) => {
    const proto = ToProto.messageStreamResult(event);
    if (!proto) {
      throw A2AError2.internalError("Stream produced an unrepresentable event.");
    }
    return StreamResponse2.toJSON(proto);
  };
...
// src/server/express/agent_card_handler.ts
function computeETag2(json) {
  const hash = import_crypto2.default.createHash("sha256").update(json).digest("hex").slice(0, 16);
  return `W/"${hash}"`;
}
...
function parseIncludeArtifacts(value) {
...
var RestTransportHandler = class _RestTransportHandler {
  ...
  async getAuthenticatedExtendedAgentCard(params, context) {
    return this.requestHandler.getAuthenticatedExtendedAgentCard(params, context);
  }
  validateSendMessageRequest(params) {
    if (!params.message) {
  ...
  async getTask(taskId, context, historyLength, tenant) {
    const params = { id: taskId, tenant: tenant || "" };
    if (historyLength !== void 0) {
      params.historyLength = this.parseHistoryLength(historyLength);
    }
    return this.requestHandler.getTask(params, context);
  }
  async cancelTask(taskId, context, tenant) {
    const params = { id: taskId, tenant: tenant || "", metadata: {} };
    return this.requestHandler.cancelTask(params, context);
  }
  async listTasks(queryParams, context) {
    const params = {
      tenant: queryParams.tenant || "",
      contextId: queryParams.contextId || "",
      status: queryParams.status ? taskStateFromJSON(
        isNaN(Number(queryParams.status)) ? queryParams.status : Number(queryParams.status)
      ) : 0 /* TASK_STATE_UNSPECIFIED */,
      pageSize: queryParams.pageSize ? Number(queryParams.pageSize) : void 0,
      pageToken: queryParams.pageToken || "",
      historyLength: queryParams.historyLength ? Number(queryParams.historyLength) : void 0,
      statusTimestampAfter: queryParams.statusTimestampAfter || void 0,
      includeArtifacts: parseIncludeArtifacts(queryParams.includeArtifacts)
    };
    return this.requestHandler.listTasks(params, context);
  }
  async resubscribe(taskId, context, tenant) {
    await this.requireCapability("streaming");
    return this.requestHandler.resubscribe({ id: taskId, tenant: tenant || "" }, context);
  }
  async createTaskPushNotificationConfig(config, context) {
    await this.requireCapability("pushNotifications");
    return this.requestHandler.createTaskPushNotificationConfig(config, context);
  }
  async listTaskPushNotificationConfigs(taskId, context, tenant) {
    const result = await this.requestHandler.listTaskPushNotificationConfigs(
  ...
  async getTaskPushNotificationConfig(taskId, configId, context, tenant) {
  ...
  async deleteTaskPushNotificationConfig(taskId, configId, context, tenant) {
    await this.requestHandler.deleteTaskPushNotificationConfig(
      { taskId, id: configId, tenant: tenant || "" },
      context
    );
...
var restErrorHandler = (err, _req, res, next) => {
  if (err instanceof SyntaxError && "body" in err) {
    return res.status(400).json(toRestErrorBody(new RequestMalformedError("Invalid JSON payload."), 400));
  }
  next(err);
};
var restContentTypeGuard = (req, res, next) => {
  const rawContentType = req.header("content-type");
  if (!rawContentType) {
    next();
    return;
  ...
  res.status(HTTP_STATUS.BAD_REQUEST).json(toRestErrorBody(error, HTTP_STATUS.BAD_REQUEST));
};
function restHandler(options) {
  ...
  router.use(
    ...
    import_express6.default.json({ type: [JSON_CONTENT_TYPE, A2A_CONTENT_TYPE], strict: false }),
    restErrorHandler
  );
  const buildContext = async (req) => {
    const user = await options.userBuilder(req);
    const tenant = req.params.tenant || void 0;
    const requestedVersion = req.header(A2A_VERSION_HEADER) || void 0;
    const ctxBuilder = options.contextBuilder ?? defaultServerCallContextBuilder;
    const context = ctxBuilder({
  ...
  const sendStreamResponse = async (res, stream, context) => {
    ...
    try {
      ...
      setExtensionsHeader(res, context);
      const statusCode = restStatusFor(error);
      res.status(statusCode).json(toRestErrorBody(error, statusCode));
      return;
    }
    Object.entries(SSE_HEADERS).forEach(([key, value]) => {
      res.setHeader(key, value);
    ...
    try {
      ...
      if (!res.writableEnded) {
        res.write(formatSSEErrorEvent(toRestErrorBody(streamError, restStatusFor(streamError))));
      }
    } finally {
      if (!res.writableEnded) {
        res.end();
  ...
  const tenantMiddleware = (req, _res, next) => {
    ...
    if (bodyTenant && bodyTenant !== pathTenant) {
      console.warn(
        `Tenant mismatch: URL path tenant "${pathTenant}" differs from request body tenant "${bodyTenant}". Using path tenant as the canonical value.`
      );
    }
    const queryTenant = req.query?.tenant;
    if (queryTenant && queryTenant !== pathTenant) {
      console.warn(
        `Tenant mismatch: URL path tenant "${pathTenant}" differs from query param tenant "${queryTenant}". Using path tenant as the canonical value.`
      );
    }
    if (req.body) {
      req.body.tenant = pathTenant;
    }
    req.query.tenant = pathTenant;
    next();
  };
  ...
  registerRoute("get", "/extendedAgentCard", async (req, res) => {
    ...
    sendResponse(res, HTTP_STATUS.OK, context, result, AgentCard);
  });
  registerRoute("post", "/message\\:send", async (req, res) => {
    ...
    const protoResult = ToProto2.messageSendResult(result);
    sendResponse(
      res,
      HTTP_STATUS.OK,
      context,
  ...
  registerRoute("post", "/tasks/:taskId\\:cancel", async (req, res) => {
    ...
    sendResponse(res, HTTP_STATUS.OK, context, result, Task);
  });
  ...
  registerRoute("get", "/tasks", async (req, res) => {
    ...
    sendResponse(res, HTTP_STATUS.OK, context, result, {
      ...ListTasksResponse,
      toJSON: (message) => serializeListTasksResponse(message, { includeArtifacts })
    });
  });
