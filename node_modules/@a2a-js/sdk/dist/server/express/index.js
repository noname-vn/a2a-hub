...
function legacyRestRouter(options) {
  ...
  router.use(
    ...
    express3.json({ type: LEGACY_JSON_CONTENT_TYPE, strict: false }),
...
function computeETag2(json) {
  const hash = crypto2.createHash("sha256").update(json).digest("hex").slice(0, 16);
  return `W/"${hash}"`;
}
...
function restHandler(options) {
  ...
  router.use(
    ...
    express5.json({ type: [JSON_CONTENT_TYPE, A2A_CONTENT_TYPE], strict: false }),
    restErrorHandler
  );
  const buildContext = async (req) => {
    const user = await options.userBuilder(req);
