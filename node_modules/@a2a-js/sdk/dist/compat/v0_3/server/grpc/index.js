...
var LEGACY_GRPC_TO_V1 = Object.freeze(
  Object.fromEntries(
    Object.entries(LEGACY_JSONRPC_TO_LEGACY_GRPC).map(([jsonRpc, grpc2]) => [
      grpc2,
...
function encodeGrpcStatusDetails(status2, message, reason, metadata) {
...
function buildGrpcErrorMetadata(Metadata2, error) {
...
function toCompatMessage(coreMsg) {
  ...
  const contextId = nonEmptyString2(coreMsg.contextId);
  if (contextId !== void 0) result.contextId = contextId;
  const taskId = nonEmptyString2(coreMsg.taskId);
  if (taskId !== void 0) result.taskId = taskId;
  const metadata = deepCloneMetadata(coreMsg.metadata);
  if (metadata !== void 0) result.metadata = metadata;
  const extensions = nonEmptyArray2(coreMsg.extensions);
  if (extensions !== void 0) result.extensions = extensions;
  const referenceTaskIds = nonEmptyArray2(coreMsg.referenceTaskIds);
...
function toCompatTaskStatusUpdateEvent(coreEvent) {
  const status2 = coreEvent.status ? toCompatTaskStatus(coreEvent.status) : { state: "unknown" };
  const result = {
    kind: "status-update",
    taskId: coreEvent.taskId,
    contextId: coreEvent.contextId,
    status: status2,
    final: FINAL_LEGACY_STATES.has(status2.state)
  };
...
// src/compat/v0_3/server/grpc/grpc_service.ts
function legacyGrpcService(options) {
...
function _envelope(params, method) {
  return { id: 0, jsonrpc: "2.0", method, params };
}
function _parseSendMessageRequest(req) {
  const params = FromProto.messageSendParams(req);
  return toCoreSendMessageRequest(_envelope(params, "message/send"));
}
function _parseTaskSubscriptionRequest(req) {
...
function _parseGetTaskRequest(req) {
  const params = FromProto.taskQueryParams(req);
  return toCoreGetTaskRequest(_envelope(params, "tasks/get"));
}
function _parseCancelTaskRequest(req) {
  const params = FromProto.taskIdParams(req);
  return toCoreCancelTaskRequest(_envelope(params, "tasks/cancel"));
}
function _parseCreateTaskPushNotificationConfigRequest(req) {
...
function _parseGetTaskPushNotificationConfigRequest(req) {
...
function _parseListTaskPushNotificationConfigRequest(req) {
...
function _parseDeleteTaskPushNotificationConfigRequest(req) {
...
function _parseGetAgentCardRequest(_req) {
...
function _serializeSendMessageResult(result) {
  if (!result) {
    throw new InvalidAgentResponseError("Invalid SendMessage result from request handler");
  }
  const compat = "messageId" in result ? toCompatMessage(result) : toCompatTask(result);
...
function _serializeStreamResponse(event) {
...
function _serializeTaskPushNotificationConfig(cfg) {
  return ToProto.taskPushNotificationConfig(toCompatTaskPushNotificationConfig(cfg));
}
function _serializeListTaskPushNotificationConfigResponse(response) {
...
var LEGACY_CODE_TO_GRPC_STATUS = {
  [-32700]: grpc.status.INVALID_ARGUMENT,
  // Parse error
...
var mapToError = (error) => {
  let code;
  if (isJsonRpcError(error) && LEGACY_CODE_TO_GRPC_STATUS[error.envelopeCode] !== void 0) {
  ...
  const result = { code, details: message };
  const md = buildGrpcErrorMetadata(grpc.Metadata, error);
  if (md) result.metadata = md;
  return result;
};
var _buildContext = async (call, userBuilder, requestHandler, contextBuilder) => {
  ...
  const requestedVersion = versionHeaders.length > 0 ? versionHeaders[0].toString() : void 0;
  const headers = {};
...
var Empty = {
  encode(_, writer = new BinaryWriter5()) {
    return writer;
  },
  decode(input, length) {
    const reader = input instanceof BinaryReader5 ? input : new BinaryReader5(input);
...
var TaskPushNotificationConfig3 = {
  encode(message, writer = new BinaryWriter8()) {
    ...
    if (message.pushNotificationConfig !== void 0) {
      PushNotificationConfig3.encode(
...
var GetAgentCardRequest2 = {
  encode(_, writer = new BinaryWriter8()) {
    return writer;
  },
  decode(input, length) {
    const reader = input instanceof BinaryReader8 ? input : new BinaryReader8(input);
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 3: {
          ...
          message.payload = {
            $case: "statusUpdate",
            value: TaskStatusUpdateEvent3.decode(reader, reader.uint32())
          };
          continue;
        }
        case 4: {
          ...
          message.payload = {
            $case: "artifactUpdate",
            value: TaskArtifactUpdateEvent3.decode(reader, reader.uint32())
          };
          continue;
        }
      }
...
var A2AServiceClient = makeGenericClientConstructor(
  A2AServiceService,
  "a2a.v1.A2AService"
);
...
export {
  A2AServiceService as LegacyA2AService,
  legacyGrpcService
};
