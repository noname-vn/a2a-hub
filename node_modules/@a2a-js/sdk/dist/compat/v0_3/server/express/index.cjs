...
__export(express_exports, {
  legacyAgentCardRouter: () => legacyAgentCardRouter,
  legacyRestRouter: () => legacyRestRouter
});
...
function legacyRestRouter(options) {
  ...
  router.use(
    ...
    import_express2.default.json({ type: LEGACY_JSON_CONTENT_TYPE, strict: false }),
  ...
  const encodeSendMessageResult = (result) => {
    ...
    return SendMessageResponse.toJSON(protoResult);
  };
  const encodeTask = (task) => {
    return Task.toJSON(ToProto.task(task));
  };
  const encodeTaskPushNotificationConfig = (cfg) => {
    return TaskPushNotificationConfig.toJSON(ToProto.taskPushNotificationConfig(cfg));
  };
  const encodeAgentCard = (card) => {
    return AgentCard.toJSON(ToProto.agentCard(card));
  };
  ...
  const decodeSendMessageRequest = (rawBody) => {
    const proto = SendMessageRequest.fromJSON(rawBody ?? {});
    return FromProto.messageSendParams(proto);
  };
  const decodeTaskPushNotificationConfig = (rawBody) => {
    const proto = TaskPushNotificationConfig.fromJSON(rawBody ?? {});
  ...
  const encodeStreamEvent = (event) => {
    ...
    return StreamResponse.toJSON(proto);
  };
