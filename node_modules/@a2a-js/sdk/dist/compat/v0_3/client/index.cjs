...
__export(client_exports, {
  LegacyJsonRpcTransport: () => LegacyJsonRpcTransport,
  LegacyRestTransport: () => LegacyRestTransport,
  isLegacyAgentCard: () => isLegacyAgentCard,
  parseLegacyAgentCard: () => parseLegacyAgentCard
});
...
var LegacyRestTransport = class _LegacyRestTransport {
  ...
  async createTaskPushNotificationConfig(params, options) {
    ...
    const body = TaskPushNotificationConfig.toJSON(protoConfig);
  ...
  async getTask(params, options) {
    ...
    const result = await this._sendRequestJson("GET", path, void 0, Task, options);
    return toCoreTask(FromProto.task(result));
  }
  async cancelTask(params, options) {
    const path = `/v1/tasks/${encodeURIComponent(params.id)}:cancel`;
    const result = await this._sendRequestJson("POST", path, void 0, Task, options);
    return toCoreTask(FromProto.task(result));
  }
  ...
  _buildSendMessageRequestJson(core) {
    ...
    return SendMessageRequest.toJSON(protoRequest);
  }
  ...
  static _processSseEventData(jsonData) {
    ...
    let protoEnvelope;
    try {
      protoEnvelope = StreamResponse.fromJSON(JSON.parse(jsonData));
    } catch (e) {
      throw new Error(
        `Failed to parse SSE event data: "${jsonData.substring(0, 100)}...". Original error: ${e instanceof Error && e.message || "Unknown error"}`,
        { cause: e }
