...
async function* parseSseStream(response, maxEventSizeBytes = DEFAULT_MAX_SSE_EVENT_SIZE_BYTES) {
  if (!response.body) {
...
function sseSizeError(what, maxEventSizeBytes) {
  return new Error(
    `${what} exceeded the maximum allowed size of ${maxEventSizeBytes} bytes. Pass maxEventSizeBytes to raise the limit, or prefer FileWithUri parts for large payloads.`
  );
}
...
// src/compat/v0_3/client/card-resolver.ts
...
// src/compat/v0_3/client/transports/json_rpc_transport.ts
...
var LegacyJsonRpcTransport = class _LegacyJsonRpcTransport {
  ...
  _fetch(...args) {
    ...
    throw new Error(
      "A `fetch` implementation was not provided and is not available in the global scope. Please provide a `fetchImpl` in the LegacyJsonRpcTransportOptions."
    );
  }
  ...
  static _processSseEventData(jsonData, originalRequestId) {
    ...
    if ("error" in legacyStreamResponse) {
      ...
      throw new Error(
        ...
        { cause: fromJsonRpcErrorResponse(legacyStreamResponse) }
      );
    }
  ...
  static _parseSendMessageResult(result) {
    ...
    throw new InvalidAgentResponseError(
      `Unexpected v0.3 message/send result kind: ${String(result.kind)}`
    );
  }
};
...
var SendMessageResponse2 = {
  fromJSON(object) {
    return {
      payload: isSet2(object.task) ? { $case: "task", value: Task2.fromJSON(object.task) } : isSet2(object.message) ? { $case: "msg", value: Message2.fromJSON(object.message) } : isSet2(object.msg) ? { $case: "msg", value: Message2.fromJSON(object.msg) } : void 0
    };
  },
  toJSON(message) {
    const obj = {};
...
// src/compat/v0_3/client/transports/rest_transport.ts
...
var LegacyRestTransport = class _LegacyRestTransport {
  ...
  async deleteTaskPushNotificationConfig(params, options) {
    const path = `/v1/tasks/${encodeURIComponent(params.taskId)}/pushNotificationConfigs/` + encodeURIComponent(params.id);
    await this._sendRequestJson("DELETE", path, void 0, void 0, options);
  }
  async getTask(params, options) {
    ...
    const result = await this._sendRequestJson("GET", path, void 0, Task2, options);
    return toCoreTask(FromProto.task(result));
  }
  async cancelTask(params, options) {
    const path = `/v1/tasks/${encodeURIComponent(params.id)}:cancel`;
    const result = await this._sendRequestJson("POST", path, void 0, Task2, options);
    return toCoreTask(FromProto.task(result));
  }
  ...
  async *resubscribeTask(params, options) {
    ...
    yield* this._sendStreamingRequest(path, "GET", void 0, options);
  }
  /**
  ...
  _fetch(...args) {
    ...
    throw new Error(
      "A `fetch` implementation was not provided and is not available in the global scope. Please provide a `fetchImpl` in the LegacyRestTransportOptions."
    );
  }
  ...
  async _sendRequestJson(method, path, body, responseType, options) {
    ...
    if (response.status === 204 || responseType === void 0) {
      return void 0;
    }
...
// src/client/transports/tenant_transport_decorator.ts
...
var JsonRpcTransport = class {
  ...
  _processSseEventData(jsonData, originalRequestId) {
    ...
    if ("error" in a2aStreamResponse) {
      ...
      throw new Error(
        `SSE event contained an error: ${err.message} (Code: ${err.code}) Data: ${JSON.stringify(err.data || {})}`,
        { cause: fromJsonRpcErrorResponse(a2aStreamResponse) }
      );
    }
...
var RestTransport = class _RestTransport {
  ...
  async deleteTaskPushNotificationConfig(params, options) {
    ...
    await this._sendRequest("DELETE", path, void 0, options, void 0, void 0);
  }
  async getTask(params, options) {
    const queryParams = new URLSearchParams();
    if (params.historyLength !== void 0) {
  ...
  async listTasks(params, options) {
    ...
    if (params.status !== void 0 && params.status !== 0 /* TASK_STATE_UNSPECIFIED */) {
  ...
  async *resubscribeTask(params, options) {
    ...
    yield* this._sendStreamingRequest(path, void 0, options);
  }
  _fetch(...args) {
    if (this.customFetchImpl) {
      return this.customFetchImpl(...args);
